...
BinarySearch.Lower = <T>(array: T[], target: T, isOrdered: (a: T, b: T) => boolean = (a: T, b: T) => a < b): number => BinarySearch.Upper(array, target, (a: T, b: T) => isOrdered(a, b) || !isOrdered(b, a)) + 1;
BinarySearch.Upper = <T>(array: T[], target: T, isOrdered: (a: T, b: T) => boolean = (a: T, b: T) => a < b): number => {
  let [begin, end] = Endpoints(array);
  let middle = midpoint(begin, end);
  while (begin < end) {
    if (isOrdered(target, array[middle])) {
